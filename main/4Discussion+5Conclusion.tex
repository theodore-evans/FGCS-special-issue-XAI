...
% For example, the two-factor counterfactual explanation was deemed to provide more valuable information than the simpler, one-factor case, but less desirable due to its added complexity. 
...
% TODO wrt below, the correlation between understandability and value across questionnaire results should be provided in the Results (and interpretations, maybe)
This is particularly challenging with respect to the user preference for \textit{intuitively understandable} explanations and explainability elements, as these inherently draw upon the prior understanding and associations of the user. As these priors vary user-to-user and case-to-case, there is a degree of uncertainty in what information will actually be communicated to the user. Also to note here, is that even elements that are not designed as explanations \textit{per se} may also be subject to this caveat. For instance, the simple trust scores demonstrated in this study function based on the implicit analogy to human, specifically, pathologist confidence in their own annotations. While both this . The intuitive understandability of such a trust score, based upon the implication of correspondence between these indicators, is therefore dangerously misleading.

% The same conflict applies to explanations designed to be intuitive to the user. Explanations that rely upon on user intuition. Such explanations may be easy to digest, whilst contain potentially ambiguous, or even misleading, implicit content. >> example??
...
The expectation of relatable explanations, which is to say, explanations that represent relatable AI reasoning, presents a similar challenge.
...
If, through this process, the usefulness and usability of explainability elements cannot be reconciled with the sources of bias they might introduce, it may be an indication that their inclusion is inappropriate for the use case at hand. In such cases, our findings suggest that it may be safer to exclude explainability altogether and live with the black-box model, rather than including potentially misleading implementations of it.

% Better not to `add explainability' in as an afterthought to AI development (as is, in the experience of the authors, often the case). Rather, that the informational content and communication modality required for the user of an AI system, in order that they are able to take the best decision possible, are considerations that must be built into the development from step one.
...
With respect to these limitations, we emphasise that selection and implementation of xAI methods presented are a reflection not only of the technical state of the art, but also as the thoughts of the machine learning researchers, AI developers and pathologists upon whose feedback they were based. We suggest that they therefore constitute a representative sample of approaches that may be `added on' to AI solutions to meet the requirement, whether real or perceived, for explainability. Moreover, the support of these findings by theoretical work, coupled with the risk of harm if not given their due consideration, make these critical points to consider in xAI development, even if they only represent the worst-case scenario.
...
There is fertile ground for further studies of this type. In particular, empirical studies of user interaction with explainability elements embedded into more true-to-life workflow would provide valuable insights. Elements of such research apparata for digital pathology might include real AI results, displayed within a fully interactive slide viewer, featuring interactive xAI components. This immersive research environment would also be augmented with gaze tracking and bio-monitoring technology in order to collect rich quantitative data on user attention and response. 

Important future work also exists in the application of these findings to xAI research and development. Our study suggest that safe and effective forms of xAI in pathology should balance usability -- of which, simplicity and relatability are important components -- against the  representing the decision-making processes and capabilities of AI system. 
